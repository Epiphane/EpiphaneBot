...
ARaid* URaidManager::StartRaid(TSubclassOf<ARaid> RaidClass)
{
    ...
    CurrentRaid = ARaid::CreateRaid(GetWorld(), RaidClass, Chat);
    if (ensure(CurrentRaid))
    {
        OnRaidCreated.Broadcast(CurrentRaid);
...
void URaidManager::GetRaidEligibility(TScriptInterface<IEpiUser> User, ERaidEligibility& Result, ARaid*& Raid, URaidParticipantComponent*& Participant, float& Cooldown)
{
    Raid = CurrentRaid;
    ...
    else
    {
        ...
        CurrentRaid->IsJoinable(User, Joinable, Participant);
